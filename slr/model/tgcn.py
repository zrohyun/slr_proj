...
class TGCN(nn.Module):
  ...
  def __init__(self,
    ...
    self.gtcn0 = nn.Conv2d(in_channel,out_c,kernel_size=(self.k,1), 
          padding=(int((self.k - 1) / 2), 0),
          stride=(self.s, 1)).to(dev)
    
    self.gtcn_layers = nn.ModuleList([
        nn.Conv2d(in_channel,out_channel, kernel_size=(self.k,1),
                  padding = (int((self.k -1)/2),0),
                  stride=(s,1)).to(dev)
  ...
  def forward(self, G):
    ...
    # A = G[G.sum((2,3)) != 0].view(B,-1,F,V).mean(1)

    
    A = torch.stack([g[g.sum((1,2)) != 0].view(-1,F,V).mean(0) for g in G],dim=0)
    # print(A.shape, type(A), A.is_cuda)
    
    # print(G.shape)
    # print(G[G.sum((2,3)) != 0].shape,type(G[G.sum((2,3)) != 0]))
    # print(G[G.sum((2,3)) != 0].view(B,-1,F,V).shape)
    ...
    for layer in self.gtcn_layers:
      ...
      x = nn.BatchNorm1d(C*V).to(self.d)(x.permute(0,1,3,2).contiguous().view(B,-1,F))
