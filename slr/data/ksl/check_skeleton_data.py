...
def get_sample_data():
    with open("sample_data.txt",'r') as f:
        ...
        ks = ['face_keypoints_2d','pose_keypoints_2d','hand_left_keypoints_2d','hand_right_keypoints_2d']
        return np.concatenate(tuple(np.array(sample_data[k]).reshape((-1,3)) for k in ks))
...
def plot_scatt(x,y, v_range, annot=True, savefig=True, fig_name = 'fig.png'):
    ...
    for i in v_range:
        annotate(
          ...
          x[i], y[i] )
    ax.invert_yaxis()
    if savefig: plt.savefig(fig_name)
    plt.show()
